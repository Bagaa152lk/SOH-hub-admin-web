...
export default function FeedbackPage() {
  ...
  const {
    ...
    loading: FLoading,
    error: FError,
  } = useQuery(GET_ALL_FEEDBACKS, { fetchPolicy: "network-only" });
  const datas = FData?.getFeedbacks;
  ...
  if (FLoading) return <MainLoading />;
  if (FError) return <p className="text-white">Error: {FError.message}</p>;
        ...
        <div className="w-full h-[56px] custom-background flex justify-between items-center px-[20px] py-[16px]">
          ...
          <div className="flex items-center gap-[10px]">
            {["All", "Positive", "Negative"].map((type) => (
              ...
              >
                {type === "Positive" ? "Урамшуулал" : null}
                {type === "Negative" ? "Гомдол" : null}
                {type === "All" ? "Бүгд" : null}
              </button>
            ))}
            {/* SEARCH */}
            ...
            {datas &&
              datas.map((feedback: any, index: number) => (
                ...
                >
                  ...
                  >
                    {feedback.type === "positive" ? "Урамшуулал" : "Гомдол"}
                  </div>
                  ...
                  <div className="flex items-center gap-[8px]">
                    ...
                    <span>
                      ...
                      {/* {feedback.toUserId.firstname} {feedback.toUserId.lastname} */}
                    </span>
                  </div>
                ...
                <div className="h-[50px] text-[14px] text-white/80 flex gap-[10px] items-center bg-white/10 rounded-[14px] px-[10px]">
                  <div className="border w-[30px] h-[30px] rounded-full relative border-[#80FFB766]">
                    <img
                      ...
                      src={
                        selectedReport?.toUserId?.profilePic
                          ? selectedReport.toUserId.profilePic
                          : "/image/profile.png"
                      }
                      alt=""
                  ...
                  <span>
                    ...
                    selectedReport?.toUserId?.lastname
                      ? `${selectedReport.toUserId.lastname} ${selectedReport.toUserId.firstname}`
                      : "Хэрэглэгч байхгүй"}
                  </span>
                </div>
              </div>
