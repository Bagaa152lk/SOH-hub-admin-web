...
export default function PropertyOwnerSosTable() {
  // State нэмэх
  const [selectedMarker, setSelectedMarker] = useState<any | null>(null);
  const [selectedSos, setSelectedSos] = useState<any | null>(null);
  const [showMaps, setShowMaps] = useState(false);
  const { data, loading, error } = useQuery(GET_SOS, {
    variables: { search: {} },
  });

  const sosDatas = data?.getsSos;
  const filteredSosData = sosDatas?.filter(
  ...
  return (
    <div className="grid grid-cols-10 w-full h-full gap-[20px] p-4">
        ...
        <div className="w-full h-[942px] overflow-hidden rounded-[18px] bg-black/10">
          <div className="w-full h-[56px] flex justify-between items-center px-[18px] bg-black/10">
            <div className="flex items-center gap-[8px]">
              ...
              <div className="text-white/90 text-[14px]">Нийт SOS</div>
              <div className="rounded-[55px] bg-white/15 text-white/50 text-[12px] font-[500] px-[15px] py-[3px]">
                {filteredSosData ? filteredSosData.length : 0}
              </div>
            </div>
          ...
          {showMaps ? (
            ...
            <div className="w-full p-[20px] flex flex-col gap-[10px] h-[calc(100%-56px)] overflow-y-auto">
              ...
              <div className="flex flex-col gap-[10px] h-[calc(100%-80px)] overflow-y-auto">
                {filteredSosData?.map((item: any, i: any) => (
                  <SosReportList
                    onClick={() => {
                      handleClick(item);
                    }}
      ...
      <div className="col-span-10 2xl:col-span-2 h-[942px]">
        {selectedSos ? (
          ...
          <div className="w-full h-full overflow-y-auto">
            <IlgeesenZorchil title="Нийт зөрчил" />
          </div>
        )}
      </div>
    </div>
