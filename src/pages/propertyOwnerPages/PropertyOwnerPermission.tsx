...
export default function PropertyOwnerPermission() {
  const [showSearch, setShowSearch] = useState(false);
  const [filterType, setFilterType] = useState("Зөрчилүүд");
  const [selectedPermission, setSelectedPermission] = useState<Feedback | null>(
    null
  );
  const { id } = useParams();
  ...
  const datas = data?.getViolations.filter(
    (da: any) => id === da?.propertyId?._id
  );

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  // const { data, loading, error } = useQuery(GET_VIOLATION);
  // const feedbacks = data?.getViolations;

  let paginatedFeedbacks;
  if (datas && datas.length > 10) {
    paginatedFeedbacks = datas.slice(
      (currentPage - 1) * itemsPerPage,
      currentPage * itemsPerPage
    );
  } else if (datas && datas.length <= 10) {
    paginatedFeedbacks = datas;
  }

  const totalPages = datas ? Math.ceil(datas.length / itemsPerPage) : 1;
        ...
        <div className="w-full h-[942px] overflow-hidden rounded-[18px] bg-black/10 flex flex-col gap-[20px] pb-[20px] relative">
          ...
          <div className="w-full h-[56px] custom-background flex justify-between items-center px-[20px] py-[16px]">
            ...
            <div className="flex items-center gap-[10px]">
              {["Зөрчилүүд", "Зөвшөөрөл"].map((type, i) => (
                <button
                  key={i}
                ...
                >
                  <div className="flex items-center gap-[42px]">
                    ...
                    <span className="text-[14px] leading-[14px] whitespace-nowrap w-[140px] overflow-hidden text-ellipsis text-center">
                      {item?.createdAt
                        ? formatDateTimeDot(new Date(item.createdAt))
                        : "-"}{" "}
                    </span>
                    <div className="w-[130px] flex justify-start">
                      <div className="rounded-[8px] flex items-center px-[15px] py-2 border border-[#80FFB766] bg-[#80FFB714]">
                        <div className="text-white/60 text-[12px] flex items-center gap-[5px]">
                          <CorrectIcon />
                          <p>Шийдсэн</p>
                        </div>
                      </div>
          ...
          <div className="h-full w-full rounded-[18px] overflow-hidden bg-black/15 ">
            ...
            <div className="p-[20px] h-[calc(100%-56px)] flex flex-col gap-4 ">
              <div className="flex flex-col gap-[10px]">
                <img
                  src="/image/hothonizurag.png"
                  className="h-[154px] w-full rounded-[16px] object-cover bg-center"
                  alt=""
                />
              ...
              <div className="flex flex-col gap-[10px]">
                <div className="w-full border border-white/15 p-[16px] flex flex-col gap-[10px] rounded-[14px]">
                  ...
                  <div className="h-[50px] text-[14px] text-white/80 flex gap-[10px] items-center bg-white/10 rounded-[14px] px-[10px]">
                    ...
                    {/* {selectedPermission?.employee?.name} */}
              ...
              <div className="w-full rounded-[14px] border border-white/10 px-4 py-[18px]">
                <p className="text-[14px] text-white/80">
                  Хүргэлтээр бичиг баримт ирсэн байгаа тул зөвшөөрч өгнө үү.
                </p>
              </div>
            </div>
          </div>
